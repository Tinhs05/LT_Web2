...
//             <img class="access-denied-img" src="../../templates/assets/cảnh báo 1.jpg" alt="">
//         </div>`
...
function getPathImage(path) {
    ...
    return "../../templates/assets/image/" + patharr[patharr.length - 1];
}
...
function chooseFileHV(fileInput) {
    if(fileInput.files && fileInput.files[0]){
        var reader = new FileReader();

        reader.onload = function(e) {
            $('#image-hv').attr('src', e.target.result);
        }
        reader.readAsDataURL(fileInput.files[0]);
    }
}
...
function uploadImage(input) {
            ...
            if (file) {
                const flname = "./templates/assets/image/"+ file.name;
...
//         <tr>
//         <td>${i + 1}</td>
//         <td><div class="prod-img-title"><img class="prd-img-tbl" src="./templates/${mergeObj[i].img}" alt=""><p id="title-prod-tk">${mergeObj[i].title.toUpperCase()}</p></div></td>
...
function togglePasswordConfirm() {
    ...
    const show_password_confirm = document.getElementById("show_password_confirm");
    const hide_password_confirm = document.getElementById("hide_password_confirm");
